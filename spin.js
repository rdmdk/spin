...
button_span = button.querySelector('span'),
edit = document.querySelector('.edit'),
colors = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p'];

let list;

if ((window.location.hash && window.location.hash !== '#') || (window.location.search && window.location.search !== '?')) {
	const h_s = window.location.hash || window.location.search;
...
function update(x) {
	const input = x ? x : window.prompt('Enter a comma-delimited list of initials', localStorage.list);
	if (input !== '' && input !== null) {
