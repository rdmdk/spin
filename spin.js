...
function spacing() {
	var l = list.length;
	var fs1 = l <= 15 ? 10 : l <= 30 ? 11.75 : l <= 45 ? 12.5 : 13;
...
function update() {
	...
	if (input !== null && input !== localStorage.list) {
		...
		else if (input === "ci") list = "ac,alg,b,da,dl,j,l,mf,mt,o,p,r,sl,sw,v";
		else list = input;
	...
	if (window.location.hash) {
		setTimeout(() => {
			...
			section.querySelectorAll("div").forEach((d) => {
				if (hashlist.indexOf(d.querySelector("span").innerText) > -1) d.parentNode.apppendChild(d);
			});
		}, 4e3);
	}
	setTimeout(() => {
